...
from auto_eval.data_utils import build_struct_data
...
def do_eval(subjects,output_path,data_type):
    ...
    for subject in tqdm(subjects):
        ...
        if os.path.exists(record_path) and not FORCE_EVAL:
            continue
        run_string = 'exec_aigc_evals {} struct_{}_{} --debug=1 --registry_path={} --log_to_file={} --record_path={}'.format(
            MODEL,
            data_type,
            subject,
            registry_path,
...
if __name__ == '__main__':
    ...
    data_type = "struct"
    # 构建数据
    subjects = build_struct_data(data_path, registry_path, data_type=data_type)
